...
test: src/github.com/coryb/gotee
	go test -v
...
update-changelog: 
	...
	echo >> CHANGELOG.md.new; \
	$(MAKE) changes | \
	perl -pe 's{\[([a-f0-9]+)\]}{[[$$1](https://github.com/coryb/gotee/commit/$$1)]}g' | \
	perl -pe 's{\#(\d+)}{[#$$1](https://github.com/coryb/gotee/issues/$$1)}g' >> CHANGELOG.md.new; \
